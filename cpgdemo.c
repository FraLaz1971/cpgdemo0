...
static void demo1()
{
  ...
  cpglab("(x)", "(y)", "Cyborg PGPLOT Example 1: y = x\\u2\\d");
  /*
...
static void demo2()
{
  ...
  cpgmtxt("t", 1.0, 0.0, 0.0, "Cyborg Contouring using cpgcont()");
...
#define NPOL  6
  ...
  char* lab[] =  {"Cyborg Fill style 1 (solid)",
		  "Cyborg Fill style 2 (outline)",
		  "Cyborg Fill style 3 (hatched)",
		  "Cyborg Fill style 4 (cross-hatched)"};
...
/* Label the graph. */
  ...
  cpgmtxt("T", -2.0, 0.5, 0.5, 
          "Cyborg PGPLOT fill area: routines cpgpoly(), cpgcirc(), cpgrect()");
